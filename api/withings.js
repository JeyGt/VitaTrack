// VitaTrack / Withings Public API connector (stable cookie-based version)
...
module.exports = async (req, res) => {
  ...
  if (action === 'status') {
    ...
    return json(res, 200, {
      ...
      connected: !!session?.refresh_token,
      lastSync: null
    });
  }
  ...
  if (action === 'connect') {
    ...
    // Signed state travels through Withings itself; no temporary state cookie is needed.
    const state = encode({ nonce, iat: Date.now() }, c.secret);
  ...
  if (action === 'callback') {
    ...
    // Withings validates the registered callback URL without OAuth parameters.
    if (!state && !code) {
      return json(res, 200, { ok: true, ready: true });
    }

    const saved = decode(state, c.secret);

    if (
      !saved ||
      !saved.nonce ||
      !saved.iat ||
      Date.now() - Number(saved.iat) > 10 * 60 * 1000 ||
      !code
    ) {
    ...
    if (d.status !== 0 || !d.body?.refresh_token) {
      return json(res, 502, { error: 'Withings token exchange failed' });
    }
    ...
    // Session lives only in the browser for now. This is deliberate in this reset version.
    setCookie(res, COOKIE, session, 60 * 60 * 24 * 365);
  ...
  let session = decode(getCookie(req, COOKIE), c.secret);

  if (!session) {
    return json(res, 401, { error: 'Not connected' });
  }
  ...
  async function refresh() {
    ...
    session.expires_at = Date.now() + Number(d.body.expires_in || 10800) * 1000;

    setCookie(res, COOKIE, encode(session, c.secret), 60 * 60 * 24 * 365);
    return session;
  }
  ...
  if (action === 'measurements') {
    ...
    if (d.status !== 0) {
      return json(res, 502, { error: 'Withings measurement request failed' });
    }

    const measurements = (d.body?.measuregrps || [])
      .map((g) => {
        const out = {
          id: g.grpid,
        ...
        for (const m of g.measures || []) {
          ...
          if (m.type === 88) out.visceralFat = v;
        }
