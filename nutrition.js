...
const BODY_COMP_METRICS={
  ...
  musclePct:{label:'Masse musculaire',unit:'%',digits:1},
  visceralFat:{label:'Graisse viscérale',unit:'',digits:1}
};
function bodyCompNumber(v){const n=Number(v);return Number.isFinite(n)?n:null;}
function bodyCompMetricValue(x,metric=bodyCompMetric){if(!x)return null;if(metric==='musclePct'){const m=bodyCompNumber(x.muscleMass),w=bodyCompNumber(x.weight);return m!==null&&w&&w>0?m/w*100:null;}return bodyCompNumber(x[metric]);}
...
function bodyCompRecordFormat(x,metric=bodyCompMetric){return bodyCompFormat(bodyCompMetricValue(x,metric),metric);}
...
function bodyCompSeries(metric=bodyCompMetric,days=bodyCompRange){const all=DATA.weights.filter(x=>bodyCompMetricValue(x,metric)!==null).slice().sort((a,b)=>weightSortValue(a)-weightSortValue(b));if(!all.length)return[];const latest=weightSortValue(all[all.length-1])||Date.now(),cut=latest-days*86400000;return all.filter(x=>weightSortValue(x)>=cut);}
function bodyCompDelta(metric,days=30){const pts=bodyCompSeries(metric,days);if(pts.length<2)return null;const first=bodyCompMetricValue(pts[0],metric),last=bodyCompMetricValue(pts[pts.length-1],metric);return first===null||last===null?null:last-first;}
function bodyCompDeltaText(metric,days=30){const d=bodyCompDelta(metric,days),cfg=BODY_COMP_METRICS[metric];if(d===null)return 'Pas assez de données';const sign=d>0?'+':'';const suffix=['bodyFat','musclePct'].includes(metric)?' pt':cfg.unit?` ${cfg.unit}`:'';return `${sign}${d.toFixed(1).replace('.',',')}${suffix} / ${days} j`;}
function setBodyCompMetric(metric){if(metric==='muscleMass')metric='musclePct';if(!BODY_COMP_METRICS[metric])return;bodyCompMetric=metric;renderBodyComposition();}
function setBodyCompRange(days){bodyCompRange=[7,30,90,365].includes(Number(days))?Number(days):30;renderBodyComposition();}
function openBodyCompGoals(){openProfileSheet();setTimeout(()=>document.getElementById('goalEditPanel')?.classList.add('open'),60);}
function bodyCompReferenceRange(metric){
  if(metric==='visceralFat')return{min:0,max:20,normalLow:0,normalHigh:5,labels:['Normal','', 'Élevé'],source:'Withings : indice viscéral 0–5 normal, au-dessus de 5 élevé.'};
  if(!['bodyFat','musclePct'].includes(metric))return null;
  const age=Number(DATA.profile?.age),sex=DATA.profile?.sex;if(!Number.isFinite(age)||!['homme','femme'].includes(sex)||age<20||age>79)return null;
  const band=age<40?0:age<60?1:2;
  const fat={femme:[[22,33],[24,34],[25,36]],homme:[[8,20],[11,22],[13,25]]};
  const muscle={femme:[[63,75.5],[62,73.5],[60,72.5]],homme:[[75,89],[73,86],[70,84]]};
  const [normalLow,normalHigh]=(metric==='bodyFat'?fat:muscle)[sex][band];
  return{min:metric==='bodyFat'?3:50,max:metric==='bodyFat'?50:95,normalLow,normalHigh,labels:['Bas','Normal','Élevé'],source:`Repère Withings · ${age<40?'20–39':age<60?'40–59':'60–79'} ans · ${sex==='femme'?'femme':'homme'} : ${String(normalLow).replace('.',',')}–${String(normalHigh).replace('.',',')} %.`};
}
function bodyCompStatus(metric,value){const n=bodyCompNumber(value);if(n===null)return{label:'Pas de donnée',tone:'neutral'};if(metric==='weight')return{label:'Suivi de ton objectif',tone:'neutral'};const r=bodyCompReferenceRange(metric);if(!r)return{label:'Repère indisponible',tone:'neutral'};if(n<r.normalLow)return{label:'Bas',tone:'low'};if(n<=r.normalHigh)return{label:'Normal',tone:'good'};return{label:'Élevé',tone:metric==='musclePct'?'good':'high'};}
function bodyCompTarget(metric){if(metric==='weight')return bodyCompNumber(DATA.objective?.targetWeight);if(metric==='bodyFat')return bodyCompNumber(DATA.objective?.targetBodyFat);if(metric==='musclePct')return bodyCompNumber(DATA.objective?.targetMusclePercent);return null;}
function bodyCompGoalText(metric,value){const target=bodyCompTarget(metric),n=bodyCompNumber(value);if(target===null)return'Définir un objectif';if(metric==='weight'&&n!==null){const remain=Math.abs(n-target);return remain<.05?`Objectif atteint · ${bodyCompFormat(target,metric)}`:`Objectif ${bodyCompFormat(target,metric)} · ${remain.toFixed(1).replace('.',',')} kg d’écart`; }return`Objectif ${bodyCompFormat(target,metric)}`;}
function bodyCompSetStatusEl(id,status){const el=document.getElementById(id);if(!el)return;el.textContent=status.label;el.className=`bodycomp-mini-status ${status.tone||'neutral'}`;}
function bodyCompScaleHtml(metric,value){const n=bodyCompNumber(value),target=bodyCompTarget(metric);if(metric==='weight'){
    const current=n,start=bodyCompNumber(DATA.profile?.startingWeight),goal=target;
    if(current===null)return{html:'<div class="bodycomp-zone neutral" style="width:100%"></div>',labels:['Départ','Aujourd’hui','Objectif'],status:'Ajoute une pesée pour situer ta progression.',reference:'Le poids est comparé à ton objectif personnel.',goalText:bodyCompGoalText(metric,current)};
    if(goal===null){return{html:`<div class="bodycomp-zone neutral" style="width:100%"></div><span class="bodycomp-marker" style="left:50%"></span>`,labels:['','Aujourd’hui',''],status:'Poids actuel',reference:'Définis un poids cible pour afficher ta progression vers l’objectif.',goalText:'Définir un objectif'};}
    const base=start!==null?start:current;let lo=Math.min(base,current,goal),hi=Math.max(base,current,goal);if(Math.abs(hi-lo)<.5){lo-=1;hi+=1;}const pad=Math.max(.7,(hi-lo)*.12);lo-=pad;hi+=pad;const pct=v=>Math.max(1,Math.min(99,(v-lo)/(hi-lo)*100));const curPct=pct(current),goalPct=pct(goal),status=Math.abs(current-goal)<.05?'Objectif atteint':current>goal?`${(current-goal).toFixed(1).replace('.',',')} kg au-dessus de l’objectif`:`${(goal-current).toFixed(1).replace('.',',')} kg sous l’objectif`;
    return{html:`<div class="bodycomp-zone neutral" style="width:100%"></div><span class="bodycomp-goal-marker" style="left:${goalPct}%"></span><span class="bodycomp-marker" style="left:${curPct}%"></span>`,labels:[`Départ ${bodyCompFormat(base,'weight')}`,`Aujourd’hui ${bodyCompFormat(current,'weight')}`,`Cible ${bodyCompFormat(goal,'weight')}`],status,reference:'Trait noir = aujourd’hui · trait violet = objectif.',goalText:bodyCompGoalText(metric,current)};
  }
  const r=bodyCompReferenceRange(metric);if(!r)return{html:'<div class="bodycomp-zone neutral" style="width:100%"></div>',labels:['Bas','Normal','Élevé'],status:'Repère indisponible',reference:'Complète l’âge et le sexe du profil pour afficher le repère Withings.',goalText:bodyCompGoalText(metric,n)};
  const range=r.max-r.min,pLow=Math.max(0,Math.min(100,(r.normalLow-r.min)/range*100)),pNorm=Math.max(0,Math.min(100,(r.normalHigh-r.normalLow)/range*100)),pHigh=Math.max(0,100-pLow-pNorm);const curPct=n===null?null:Math.max(1,Math.min(99,(n-r.min)/range*100)),goalPct=target===null?null:Math.max(1,Math.min(99,(target-r.min)/range*100)),st=bodyCompStatus(metric,n);
  const highClass=metric==='musclePct'?'muscle-high':'high';const zones=`<div class="bodycomp-zone low" style="width:${pLow}%"></div><div class="bodycomp-zone normal" style="width:${pNorm}%"></div><div class="bodycomp-zone ${highClass}" style="width:${pHigh}%"></div>`;
  const markers=`${goalPct===null?'':`<span class="bodycomp-goal-marker" style="left:${goalPct}%"></span>`}${curPct===null?'':`<span class="bodycomp-marker" style="left:${curPct}%"></span>`}`;
  return{html:zones+markers,labels:r.labels,status:n===null?'Pas de donnée':`${st.label} · ${bodyCompFormat(n,metric)}`,reference:r.source+(target!==null?' Trait violet = ton objectif.':''),goalText:bodyCompGoalText(metric,n)};
}
function renderBodyCompPosition(latest){const value=latest?bodyCompMetricValue(latest,bodyCompMetric):null,scale=bodyCompScaleHtml(bodyCompMetric,value);const el=document.getElementById('bodyCompScale');if(el)el.innerHTML=scale.html;const labels=document.getElementById('bodyCompScaleLabels');if(labels)labels.innerHTML=scale.labels.map(x=>`<span>${x}</span>`).join('');setText('bodyCompPositionStatus',scale.status);setText('bodyCompReference',scale.reference);setText('bodyCompPositionGoal',scale.goalText);}
...
function renderBodyComposition(){const panel=document.getElementById('bodyCompPanel');if(!panel)return;const latest=latestWeightRecord();setText('bodyCompLastDate',latest?`${formatWeightRecordDate(latest)}${latest.source==='withings'?' · Withings':''}`:'Ajoute une pesée pour commencer');setText('bodyCompWeight',latest?bodyCompRecordFormat(latest,'weight'):'—');setText('bodyCompFat',latest?bodyCompRecordFormat(latest,'bodyFat'):'—');setText('bodyCompMuscle',latest?bodyCompRecordFormat(latest,'musclePct'):'—');setText('bodyCompVisceral',latest?bodyCompRecordFormat(latest,'visceralFat'):'—');setText('bodyCompWeightDelta',bodyCompDeltaText('weight',30));setText('bodyCompWeightObjective',bodyCompGoalText('weight',latest?bodyCompMetricValue(latest,'weight'):null));setText('bodyCompFatDelta',bodyCompDeltaText('bodyFat',30));setText('bodyCompMuscleDelta',bodyCompDeltaText('musclePct',30));setText('bodyCompVisceralDelta',bodyCompDelta('visceralFat',30)===null?'indice':bodyCompDeltaText('visceralFat',30));bodyCompSetStatusEl('bodyCompFatStatus',bodyCompStatus('bodyFat',latest?bodyCompMetricValue(latest,'bodyFat'):null));bodyCompSetStatusEl('bodyCompMuscleStatus',bodyCompStatus('musclePct',latest?bodyCompMetricValue(latest,'musclePct'):null));bodyCompSetStatusEl('bodyCompVisceralStatus',bodyCompStatus('visceralFat',latest?bodyCompMetricValue(latest,'visceralFat'):null));document.querySelectorAll('[data-bodycomp-metric]').forEach(b=>b.classList.toggle('active',b.dataset.bodycompMetric===bodyCompMetric));document.querySelectorAll('[data-bodycomp-card]').forEach(b=>b.classList.toggle('active',b.dataset.bodycompCard===bodyCompMetric));document.querySelectorAll('[data-bodycomp-range]').forEach(b=>b.classList.toggle('active',Number(b.dataset.bodycompRange)===bodyCompRange));renderBodyCompPosition(latest);renderBodyCompChart();}
function renderBodyCompChart(){const svg=document.getElementById('bodyCompChart'),empty=document.getElementById('bodyCompChartEmpty');if(!svg)return;const cfg=BODY_COMP_METRICS[bodyCompMetric],pts=bodyCompSeries(bodyCompMetric,bodyCompRange);const latest=pts.length?pts[pts.length-1]:null;setText('bodyCompChartMetricLabel',cfg.label);setText('bodyCompChartValue',latest?bodyCompRecordFormat(latest,bodyCompMetric):'—');setText('bodyCompChartTrend',bodyCompDeltaText(bodyCompMetric,bodyCompRange));const d30=bodyCompDelta(bodyCompMetric,30);const insight=document.getElementById('bodyCompInsight');if(insight){const current=latest?bodyCompMetricValue(latest,bodyCompMetric):null,status=bodyCompStatus(bodyCompMetric,current),target=bodyCompTarget(bodyCompMetric);let trend=d30===null?'Pas encore assez de mesures pour calculer une tendance sur 30 jours.':Math.abs(d30)<0.05?'La tendance sur 30 jours est globalement stable.':`Sur 30 jours, ${cfg.label.toLowerCase()} ${d30<0?'baisse':'augmente'} de ${Math.abs(d30).toFixed(1).replace('.',',')}${['bodyFat','musclePct'].includes(bodyCompMetric)?' point':cfg.unit?' '+cfg.unit:''}.`;let position=bodyCompMetric==='weight'?(target!==null?` ${bodyCompGoalText('weight',current)}.`:''):current!==null&&status.label!=='Repère indisponible'?` Position actuelle : ${status.label.toLowerCase()}.`:'';insight.innerHTML=`<strong>Lecture rapide :</strong> ${trend}${position} <span>Regarde surtout l’évolution sur plusieurs pesées.</span>`;}if(pts.length<2){svg.innerHTML='';if(empty){empty.style.display='flex';empty.textContent=pts.length===1?'Encore une mesure et la tendance apparaîtra.':'Aucune donnée pour cette mesure sur la période.';}return;}if(empty)empty.style.display='none';const W=340,H=154,padL=18,padR=10,padT=15,padB=23;const vals=pts.map(p=>bodyCompMetricValue(p,bodyCompMetric)).filter(v=>v!==null);const ref=bodyCompReferenceRange(bodyCompMetric),target=bodyCompTarget(bodyCompMetric);let min=Math.min(...vals),max=Math.max(...vals);if(ref&&bodyCompMetric!=='weight'){min=Math.min(min,ref.normalLow);max=Math.max(max,ref.normalHigh);}if(target!==null){min=Math.min(min,target);max=Math.max(max,target);}let span=max-min;if(span<0.0001)span=Math.max(1,Math.abs(max)*0.02);const margin=Math.max(span*.18,bodyCompMetric==='weight'?.25:['bodyFat','musclePct'].includes(bodyCompMetric)?.3:.15);min-=margin;max+=margin;const firstT=weightSortValue(pts[0]),lastT=weightSortValue(pts[pts.length-1]),tSpan=Math.max(1,lastT-firstT);const X=p=>padL+(weightSortValue(p)-firstT)/tSpan*(W-padL-padR),Y=v=>padT+(max-v)/(max-min)*(H-padT-padB);const path=pts.map((p,i)=>`${i?'L':'M'}${X(p).toFixed(1)},${Y(bodyCompMetricValue(p,bodyCompMetric)).toFixed(1)}`).join(' ');const area=`${path} L${X(pts[pts.length-1]).toFixed(1)},${H-padB} L${X(pts[0]).toFixed(1)},${H-padB} Z`;const fmtDate=x=>{try{return new Intl.DateTimeFormat('fr-FR',{day:'2-digit',month:'short'}).format(new Date(weightSortValue(x)));}catch(e){return'';}};const grid=[0,.5,1].map(r=>{const yy=padT+r*(H-padT-padB);return `<line x1="${padL}" y1="${yy}" x2="${W-padR}" y2="${yy}" stroke="var(--border)" stroke-width="1"/>`;}).join('');let refBand='';if(ref&&bodyCompMetric!=='weight'){const y1=Y(ref.normalHigh),y2=Y(ref.normalLow);refBand=`<rect x="${padL}" y="${Math.min(y1,y2).toFixed(1)}" width="${W-padL-padR}" height="${Math.abs(y2-y1).toFixed(1)}" fill="#78caa5" opacity=".10"/>`;}let targetLine='';if(target!==null&&target>=min&&target<=max){const ty=Y(target);targetLine=`<line x1="${padL}" y1="${ty.toFixed(1)}" x2="${W-padR}" y2="${ty.toFixed(1)}" stroke="var(--primary)" stroke-width="1.5" stroke-dasharray="4 4" opacity=".8"/><text x="${W-padR}" y="${Math.max(10,ty-4).toFixed(1)}" fill="var(--primary)" font-size="8.5" text-anchor="end">objectif</text>`;}svg.innerHTML=`${refBand}${grid}${targetLine}<path d="${area}" fill="var(--primary)" opacity=".08"/><path d="${path}" fill="none" stroke="var(--primary)" stroke-width="3" vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round"/>${pts.map(p=>`<circle cx="${X(p).toFixed(1)}" cy="${Y(bodyCompMetricValue(p,bodyCompMetric)).toFixed(1)}" r="3.3" fill="var(--surface)" stroke="var(--primary)" stroke-width="2" vector-effect="non-scaling-stroke"><title>${formatWeightRecordDate(p)} · ${bodyCompRecordFormat(p,bodyCompMetric)}</title></circle>`).join('')}<text x="${padL}" y="${H-6}" fill="var(--ink-soft)" font-size="9">${fmtDate(pts[0])}</text><text x="${W-padR}" y="${H-6}" fill="var(--ink-soft)" font-size="9" text-anchor="end">${fmtDate(pts[pts.length-1])}</text>`;}
